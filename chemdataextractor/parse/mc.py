...
mc = (Optional(left_bracket_parentheses).hide() + Optional(cem + optdelim) + values + units + Optional(
    right_bracket_parentheses).hide())('mc')

mediums = (I('saliva') | I('urine') | I('blood') | I('serum') | I('plasma'))('medium')

infix_words = W('was') | W('were') | W('of') | W('concentration') | W('concentrations') | W('normal') | W('in') | W(
    'level') | W('levels') | W('increased') | W('to') | W('are')
...
normal_range = (left_bracket_parentheses.hide() + ((W('normal') + W('range')).hide() | W('NR')) + optdelim + values)(
    'normal_range')
mc_phrase_cem_first = (Optional(mediums) + cem + infix_words_opt + mc + Optional(normal_range))('mc_phrase')
mc_phrase_cem_last = (mc + infix_words_opt + Optional(mediums) + cem + Optional(SkipTo(W('in') + mediums)) + Optional(
    normal_range))('mc_phrase')
mc_phrase_list = (Optional(mediums) + infix_words_opt + OneOrMore(cem + mc + delim))('mc_phrase')
mc_phrase_and = (mc + cem + W('and').hide() + mc + cem + W('in').hide() + mediums)('mc_phrase')

mc_phrase = mc_phrase_and | mc_phrase_list | mc_phrase_cem_last | mc_phrase_cem_first
